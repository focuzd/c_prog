...
When called with `mode: generate`, you receive:
- `knowledge-base.md`  
- `queue.json` (already filtered to due questions by srs.py)

Output a markdown-formatted review session issue body:

```markdown
# Review Session — {DATE}

> Answer each question in a comment below. One comment per question is fine, label them Q1, Q2, etc.
> Be specific. Show your reasoning, not just the answer.

---

## Q1 — {concept} (Chapter {N})
...
When called with `mode: grade`, you receive:
- The original questions (from queue.json entries)
- The learner's answers (from issue comments)
